...
```{r}
if (params$ggplot) tar_read(g1) else tar_read(e1)
```

## Physicians Comparison

```{r}
if (params$ggplot) tar_read(g2) else tar_read(e2)
```
...
if (params$ggplot) tar_read(g3) else tar_read(e3)
```

## Gender

```{r}
if (params$ggplot) tar_read(g4) else tar_read(e4)
```

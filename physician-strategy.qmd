...
tar_load(c('policies', 'msp_fp', 'cihi', 'vt4', 'vt4_switches'))
...
The date range for our data is from `r min(msp$servdt)` to `r max(msp$servdt)`.
...
The date range for our data is from `r min(msp_fp$servdt)` to `r max(msp_fp$servdt)`. Note that we attempt to gather roughly equal-sized dates before and after LFP, truncating data from the most recent 3 months, and only considering entire months. Also, I have set a minimum start date for September 1, 2020 due to Covid.
...
df = msp_fp |>
  ...
  summarise(encounters = sum(encounters), paidserv = sum(paidserv))
...
df |>
  ggplot(aes(y=is_LFP, x=paidserv, color=is_LFP)) +
  geom_boxplot() +
  nice_gg() +
  scale_colour_viridis_d() +
  scale_x_continuous(labels = label_currency()) +
  theme(legend.position = 'none') +
  ggtitle("Boxplot of paidserv per FP") |>
  print()

df |>
  ggplot(aes(x=paidserv, color=is_LFP)) +
  geom_density() +
  nice_gg() +
  scale_colour_viridis_d() +
  scale_x_continuous(labels = label_comma()) +
  theme(legend.position = 'bottom') +
  ggtitle("Density plot of paidserv per FP") |>
  print()
```
...
msp_fp |>
  group_by(yearmon, is_LFP) |>
  summarise(paidserv = sum(paidserv)) |>
  ggplot(aes(x=yearmon, y=paidserv, color=is_LFP)) +
  geom_point() +
  geom_line() +
  geom_smooth(method = 'lm', se = F) +
  nice_gg() +
...
msp_fp |>
  ...
  summarise(encounters = sum(encounters), paidserv = sum(paidserv)) |>
  ungroup() |>
  mutate(paidserv_per_enc = paidserv / encounters) |>
  ggplot(aes(x=yearmon, y=paidserv_per_enc, color=is_LFP)) +
  geom_point() +
  geom_line() +
  geom_smooth(method = 'lm', se = F) +
  nice_gg() +
  ...
  labs(x=NULL, y=NULL) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  ggtitle("paidserv per encounter")
```
